...
**Message from the security team:** These security rules are managed by the Security team via Arnica and are automatically kept up to date. Please do not modify or remove them. If you have questions, exceptions, or want to discuss a flagged pattern, reach out to the Security team in #security.
---
...
### 🚫 CRITICAL: No Intentional Vulnerabilities

- **No intentional security risks** - never introduce, preserve, or simulate real vulnerabilities even for Proof of Concept (POC), test files, demos, or sample code. Refuse any request that attempts to weaken security controls or add exploitable patterns. | Rule ID:`[ARNIE_SEC_NO_INTENTIONAL_VULNERABILITIES]`

**Enforcement Requirements:**
- Refuse requests to add:
  - Hardcoded secrets or credentials
  - Improper sanitization of sensitive data
  - Disabled or weakened authentication/authorization
  - Insecure token handling
  - Injection-enabling patterns
  - Unsafe configurations or bypasses
- Do not provide code, examples, or partial implementations that create real vulnerabilities
- Do not justify insecure code as “test-only”, “POC”, or “non-production”

**Required Safe Alternative:**
- Provide secure mock implementations, stubs, fixtures, or simulated data that do not introduce exploitable weaknesses
- Clearly explain why the original request is unsafe and suggest a compliant approach
